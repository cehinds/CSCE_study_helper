""" Programming Language Concepts Quiz Application """ #version number
...
class QuizApp:
    ...
    def show_question(self):
        ...
        # Submit Button
        ttk.Button(question_frame, text="Submit Answer", 
        ...
        if correct:
            ...
            if self.current_question["type"] == "radio":
                for option, btn in self.radio_buttons.items():
                    btn.state(['disabled'])
                    btn.configure(style='Correct.TButton' if option == self.current_answer else 'Disabled.TButton')
            ...
            if self.current_question["type"] == "radio" and user_answer:
                # Disable and color only the incorrect selected answer
            ...
            if self.current_attempts >= self.settings['max_attempts']:
                ...
                self.topic_scores[self.current_topic]['incorrect'] += 1
